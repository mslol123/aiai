...
public class  SearchController {
    ...
    public String search(String keyword, Model model,Integer pageNo){
        ...
        pageBean.setTotalPage(bookMessageList.size());
        if( pageNo == null || pageNo <= 0){
            pageNo = 1;
        }
        pageBean.setCurrentPage(pageNo);
        int pageNoLast = pageNo*8;
        if(pageNoLast>bookMessageList.size()){
            pageNoLast = bookMessageList.size();
        }
        List<BookSearch> bookSearches = bookMessageList.subList((pageNo - 1)*8, pageNoLast);
        int totalPage = bookMessageList.size()%8==0 ? bookMessageList.size()/8 : bookMessageList.size()/8+1;
